...
class HtmlDocument
{
	...
	protected function decode()
	{
		foreach($this->nodes as $node) {
			if (is_object($node) && isset($node->_[HtmlNode::HDOM_INFO_TEXT])) {
			...
			if (is_object($node) && isset($node->_[HtmlNode::HDOM_INFO_INNER])) {
			...
			if (is_object($node) && isset($node->attr) && is_array($node->attr)) {
